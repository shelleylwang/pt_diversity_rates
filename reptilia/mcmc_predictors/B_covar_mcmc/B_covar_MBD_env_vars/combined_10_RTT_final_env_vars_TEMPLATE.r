...
####################################################### CHANGE FILE PATH
pdf(file='C:/Users/SimoesLabAdmin/Documents/BDNN_Arielli/temnospondyli/mcmc_fixshift_predictors/C_covar/C_covar_MBD_env_vars/combined_10_RTT_final.pdf',width=0.6*25, height=0.6*10)
library(scales)
...
# clade_2 = "map_scaled_translated"
# clade_3 = "mat_scaled_translated"
# clade_4 = "wmmcmm_scaled_translated"

# According to any backscale.txt (env variables are all the same values in reptilia_backscale.txt, synapsida, temnospondyli)
# "map_scaled_translated" pre-scaling values: mean = 480.7703066666666, sd = 65.186473
# "mat_scaled_translated" pre-scaling values: mean = 4.089369, sd = 5.785585
# "wmmcmm_scaled_translated" pre-scaling values: mean = 37.092737, sd = 4.669959

# Defining these as variables:
clade_2_mean = 480.7703066666666
clade_2_sd = 65.186473
clade_3_mean = 4.089369
clade_3_sd = 5.785585
clade_4_mean = 37.092737
clade_4_sd = 4.669959
...
print("map_scaled_translated z-transformed values:")
print(clade_2[1:5])

# Print the first five items in clade_3, and a message indicating that the values are z-transformed
print("mat_scaled_translated z-transformed values:")
print(clade_3[1:5])

# Print the first five items in clade_4, and a message indicating that the values are z-transformed
print("wmmcmm_scaled_translated z-transformed values:")
print(clade_4[1:5])
...
clade_4 = clade_4 * clade_4_sd + clade_4_mean

# Print the first five items in clade_2, and a message indicating that the values are reverse z-transformed
print("map_scaled_translated reverse z-transformed values (map):")
print(clade_2[1:5])

# Print the first five items in clade_3, and a message indicating that the values are reverse z-transformed
print("mat_scaled_translated reverse z-transformed values (mat):")
print(clade_3[1:5])

# Print the first five items in clade_4, and a message indicating that the values are reverse z-transformed
print("wmmcmm_scaled_translated reverse z-transformed values (wmmcmm):")
print(clade_4[1:5])
...
######################################################### ADD t = VECTOR
time = -t ########################### REMOVE THE -175 IF MODEL RUN WAS NOT TRANSLATED BY -175
...
plot(clade_2[clade_1>0] ~ time[clade_1>0],type="l", 
     xaxt='n',main = "Trajectory of variable: map_scaled_translated",xlab="Time (Ma)",ylab="Rescaled value",xlim=XLIM)
axis(1, at=x_ticks, labels=x_tick_labels) # Custom x-axis with 10MY interval ticks
# abline(v=-c(65,200,251,367,445),lty=2,col="gray")
# Add vertical lines for mass extinction events
abline(v=perm_trias_extinction, col="red", lty=2) 
...
plot(clade_3[clade_1>0] ~ time[clade_1>0],type="l", 
     xaxt = 'n',main = "Trajectory of variable: mat_scaled_translated",xlab="Time (Ma)",ylab="Rescaled value",xlim=XLIM)
axis(1, at=x_ticks, labels=x_tick_labels) # Custom x-axis with 10MY interval ticks
# abline(v=-c(65,200,251,367,445),lty=2,col="gray")
# Add vertical lines for mass extinction events
abline(v=perm_trias_extinction, col="red", lty=2) 
...
abline(v=perm_trias_extinction, col="red", lty=2) 
abline(v=guadalupian_extinction, col="red", lty=2)
plot(clade_4[clade_1>0] ~ time[clade_1>0],type="l", 
     xaxt='n', main = "Trajectory of variable: wmmcmm_scaled_translated",xlab="Time (Ma)",ylab="Rescaled value",xlim=XLIM)
axis(1, at=x_ticks, labels=x_tick_labels) # Custom x-axis with 10MY interval ticks
# abline(v=-c(65,200,251,367,445),lty=2,col="gray")
# Add vertical lines for mass extinction events
abline(v=perm_trias_extinction, col="red", lty=2) 
