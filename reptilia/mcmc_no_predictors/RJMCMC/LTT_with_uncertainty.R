...
Path <- "C:/Users/SimoesLabAdmin/Documents/BDNN_Arielli/reptilia/mcmc_no_predictors/RJMCMC"
...
for (i in 1:NumReplicates) {
  ...
  # Construct file path
  file_path <- file.path(Path, sprintf('reptilia_pyrate_%d_Grj_mcmc.log', i))
  
  # Read with explicit encoding
  McmcLog <- read.table(file_path,
                        header = TRUE, 
                        sep = '\t',
                        encoding = "UTF-8",
...
df <- data.frame(
  Time = TimeVecLtt,
  Mean = LttMean,
  CI95_lower = LttCI95[1,],
  CI95_upper = LttCI95[2,],
  CI75_lower = LttCI75[1,],
  CI75_upper = LttCI75[2,]
)
...
# Create the plot
p <- ggplot(df[NotZero,], aes(x = -Time)) +
  # Add geological time scale
  coord_geo(xlim = c(-max(TimeVecLtt), 0), 
           expand = FALSE, 
           clip = "on",
           ...
           neg = T) +
  # 95% CI
  geom_ribbon(aes(ymin = CI95_lower, ymax = CI95_upper),
              fill = "purple", alpha = 0.15) +
  # 75% CI
  geom_ribbon(aes(ymin = CI75_lower, ymax = CI75_upper),
              fill = "purple", alpha = 0.15) +
  # Mean line
  geom_step(aes(y = Mean), color = "purple", size = 1) +
  # Add dashed vertical lines for major boundaries
  geom_vline(xintercept = c(-65, -200, -251, -367, -445), 
             linetype = "dashed", color = "gray") +
  # Styling
  scale_x_continuous(
    limits = c(-max(TimeVecLtt), 0),
    breaks = seq(-ceiling(max(TimeVecLtt)), 0, by = 10),
...
# Save plot
ggsave("./ltt_with_uncertainty.pdf", p, width = 20, height = 20)