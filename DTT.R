...
Note: 
  ...
  - The script will process all files ending in 'mcmc.log' in the specified directory, 
    excluding any files containing 'combined' in their filename\n")
...
all_files <- list.files(path = args$path, pattern = "mcmc\\.log$", full.names = TRUE)
...
estimated_total_curves <- length(mcmc_files) * args$thin_to
...
max_curves <- length(mcmc_files) * args$thin_to
Ltt <- matrix(NA_real_,
              ncol = max_curves,
