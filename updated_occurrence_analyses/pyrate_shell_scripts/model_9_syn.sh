#!/bin/bash
#SBATCH --job-name=model_9_syn_offset          # Job name
...
#SBATCH --output=/scratch/gpfs/sw8569/model_9_syn_offset_%j_%A_%a.out
#SBATCH --error=/scratch/gpfs/sw8569/model_9_syn_offset_%j_%A_%a.err
...
python ../PyRate/PyRate.py updated_occurrence_analyses/data/Perm-Trias/pt_synapsida_pyrate_input_txt_PyRate.py  \
 ...
 -BDNNtimevar updated_occurrence_analyses/data/AF_climactic_data/BRIDGE_and_binary_stages_data/syn_BRIDGE_stages_timevar_1myr_offset.txt \
 -BDNNtimetrait 0 -translate -175 -out _model_9_offset -wd updated_occurrence_analyses/model_9/synapsida_offset -n 200000000 -s 1000 -j ${SLURM_ARRAY_TASK_ID}