#!/bin/bash
#SBATCH --job-name=model_9_syn_new_new_dates         # Job name
...
#SBATCH --output=/workdir/pt_diversity_rates/err_out/model_9_syn_new_new_dates_%j_%A_%a.out
#SBATCH --error=/workdir/pt_diversity_rates/err_out/model_9_syn_new_new_dates_%j_%A_%a.err
...
python ../PyRate/PyRate.py updated_occurrence_analyses/data/Perm-Trias/pt_synapsida_pyrate_input_txt_PyRate.py  \
 ...
 -BDNNtimevar updated_occurrence_analyses/data/AF_climatic_data/BRIDGE_and_binary_stages_data_new_dates/syn_BRIDGE_stages_timevar_1myr_new_dates.txt \
 -BDNNtimetrait 0 -translate -175 -out _model_9_new_dates -wd updated_occurrence_analyses/model_9/synapsida_new_dates -n 200000000 -s 1000 -j ${SLURM_ARRAY_TASK_ID}