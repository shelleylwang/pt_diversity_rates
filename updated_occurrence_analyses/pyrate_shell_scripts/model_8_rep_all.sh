#!/bin/bash
#SBATCH --job-name=model_8_rep_all_offset          # Job name
...
#SBATCH --output=/scratch/gpfs/sw8569/model_8_rep_all_offset_%j_%A_%a.out
#SBATCH --error=/scratch/gpfs/sw8569/model_8_rep_all_offset_%j_%A_%a.err
...
python ../PyRate/PyRate.py updated_occurrence_analyses/data/Perm-Trias/pt_reptilia_all_pyrate_input_txt_PyRate.py  \
 ...
 -BDNNtimevar updated_occurrence_analyses/data/Perm-Trias/songEA_isotopic_data_pt_1myr/isotopic_1myr_binary_stages_timevar_offset.txt \
 -BDNNtimetrait 0 -translate -175 -out _model_8_offset -wd updated_occurrence_analyses/model_8/reptilia_all_offset -n 200000000 -s 1000 -j ${SLURM_ARRAY_TASK_ID} 
